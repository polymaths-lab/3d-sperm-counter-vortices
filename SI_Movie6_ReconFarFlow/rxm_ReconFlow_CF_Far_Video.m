function rxm_ReconFlow_CF_Far_Video

% This function generates a video containing the instant flow streamlines
% of the reconstructed human sperm's far flow.
...
fig = 1;
figure(fig) 
v=VideoWriter('FarFlow_sp8_NoBound_test1','MPEG-4');
v.FrameRate=50;
open(v);     
...
%================================================================================
    ...
    view([155 15]);  
    XL=[xl_field(1)-2 xl_field(2)+2.5]; YL=[yl_field(1)-2 yl_field(2)+2]; ZL=[zl_field(1)-2.2 zl_field(2)+2.5];
    ...
    if 1 % arrows located at the ends of the flow field. ==> clear visualization. ==> chosen one.
    ...
    c.Ticks=[0 0.1]; c.TickLabels={'0','0.1'}; 
    c.FontName = 'times'; c.TickLabelInterpreter='latex'; c.FontSize=fs; 
    text(-15,18.5,-5.5,{'Flow velocity (\mum/s)'},'FontName','times','FontSize',fs,'rotation',90);  %'FontWeight','bold'   
...
%================================================================================
    ...
    text(2,-0.3,1.3,{['Time: ',num2str(instant_nt0),'s']},'FontName','times','FontSize',fs+10,'FontWeight','bold');   %1.8,-0.3,1.3,
...
%================================================================================
    ...
    hold on; set(gcf,'color','w'); axis equal; view([0 0]); axis off; 
    xlim([xl_field(1)-2 xl_field(2)+2]); ylim([yl_field(1)-2 yl_field(2)+2]); zlim([zl_field(1)-2 zl_field(2)+2]); 
    text(-1,-1,-12,{'$x_c$'},'FontName','times','interpreter','latex','FontSize',fs);   
    ...
    hold on; set(gcf,'color','w'); axis equal; view([0 90]); axis off;
    xlim([xl_field(1)-2 xl_field(2)+2]); ylim([yl_field(1)-2 yl_field(2)+2]); zlim([zl_field(1)-2 zl_field(2)+2]); 
    text(-1,-12,0.5,{'$x_c$'},'FontName','times','interpreter','latex','FontSize',fs);   
    text(-15,0,0.5,{'$y_c$'},'FontName','times','interpreter','latex','FontSize',fs);   
    text(-14.5,10,0,{'I'},'FontName','times','interpreter','latex','FontSize',fs);   
    % Plot streamslice.
    ss=streamslice(Xg,Yg,Zg,Ug_nt0,Vg_nt0,Wg_nt0,[],[],[0],0.5);
    set(ss,'color','white','linewidth',3);   
    slice(Xg,Yg,Zg,vel_dim,[],[],[0]);
