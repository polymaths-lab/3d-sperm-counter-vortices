...
p1 = plot(t_exp,wu_exp(1,:),'k','linewidth',4);
p2 = plot(t_exp,wu_num_1(1,:),'r','linewidth',2);
p3 = plot(t_exp,wu_num_2(1,:),'g*','linewidth',1);
...
if 0
    lgd = legend([p1 p2 p3],{'$Exp.$','$Rec.(\infty)$','$Rec.(0.2L)$'},'interpreter','latex'); 
...
hold on; box on;
ax = gca; ax.TickLabelInterpreter = 'latex'; ax.FontSize = fs;
plot(lags_NB(1,:),cc_NB(1,:),'k','linewidth',2)
plot(lags_NB(2,:),cc_NB(2,:),'r','linewidth',2)
plot(lags_NB(3,:),cc_NB(3,:),'g','linewidth',2)
plot(lags_NB(4,:),cc_NB(4,:),'b','linewidth',2)
plot(lags_NB(5,:),cc_NB(5,:),'m','linewidth',2)
plot(lags_NB(6,:),cc_NB(6,:),'c','linewidth',2)
