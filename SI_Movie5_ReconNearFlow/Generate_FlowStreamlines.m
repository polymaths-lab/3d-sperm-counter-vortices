...
for i_sl = 1:nsl
    ...
    nv = nv+nvert_sl(i_sl);
    vert_temp = { vert{i_sl} };
    %h = streamtube(vert_temp ,0.01); % for near-field instant flow (no-boundary sp#8)
    h = streamtube(vert_temp ,0.03); %0.05 % for near-field instant flow (no-boundary sp#8) --> video
    %h = streamtube(vert_temp ,0.3); % for far-field instant flow (no-boundary sp#8)
