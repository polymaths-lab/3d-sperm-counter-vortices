function rxm_ReconFlow_AverageNear_Plot
...
if 1
    ...
    %view([170 10]); % for cross-section flow: proximal and middle. 
    %view([208 10]); % for cross-section flow: distal. 
    
    XL=[min(Xg(:)) max(Xg(:))];  
    YL=[-0.4, 0.4]; ZL=[-0.4, 0.2];  % for full flow: no boundary.
    %YL=[-0.8, 1]; ZL=[-bound_height, 0.5];  % for full flow: with boundary height 0.2L, 0.5L, L.
    %YL=[min(Yg(:))  max(Yg(:))]; ZL=[min(Zg(:)) max(Zg(:))];   %for cross-section flow: flow.
...
if 1   
    ...
    %[sx_3D,sy_3D,sz_3D] = meshgrid(xg(16),yg(1:1:end),zg(1:1:end)); % without boundary, vortex cross sections: xg=16. view([170 10]). x=0 (proximal).
    %[sx_3D,sy_3D,sz_3D] = meshgrid(xg(11),yg(1:1:end),zg(1:1:end)); % without boundary, vortex cross sections: xg=11. view([190 10]). x=-0.5 (middle).
    %[sx_3D,sy_3D,sz_3D] = meshgrid(xg(6),yg(1:1:end),zg(1:1:end)); % without boundary, vortex cross sections: xg=6. view([190 10]). x=-1 (distal).
    ...
    %O = [0 0 0]; %for cross section: proximal, middle, distal.
    Cx=[1; 0; 0]; 
    Cy=[0; 1; 0]; 
    Cz=[0; 0; 1]; 
    C=[Cx Cy Cz];
...
if 0
    ...
    ss=streamslice(Xg,Yg,Zg,6*Ug_mean,6*Vg_mean,6*Wg_mean,[0],[],[],0.3); 
