...
for i_sl = 1:nsl
    ...
    nv = nv+nvert_sl(i_sl);
    vert_temp = { vert{i_sl} };
    h = streamtube(vert_temp ,0.01); % for near-field full flow.  
    %h = streamtube(vert_temp ,0.05); % for near-field cross-sectional flow.
    %h = streamtube(vert_temp ,0.3); % for far-field full flow.  
