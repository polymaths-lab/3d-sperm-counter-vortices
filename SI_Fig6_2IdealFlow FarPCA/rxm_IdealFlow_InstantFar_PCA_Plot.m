function rxm_IdealFlow_InstantFar_PCA_Plot
...
if 1
    ...
    nt0 = 350; % for the datasets of 'FarFlow_CF_alpha0.6_Tfine.mat' and 'NearFlow_CF_alpha0.6_Tfine.mat'.
    ...
    elseif op==2 || op==3 % PCA-reconstructed flow.
        load FarFlow_CF_alpha0.6_Tfine_PCA_Accuracy99.99.mat;
    ...
    view([-35 20]); % for all instant far flows of the idealized model.
    %xlim([xl_field(1),xl_field(2)]); ylim([yl_field(1),yl_field(2)]); zlim([zl_field(1),zl_field(2)]); % for raw/ recon. flows.
    xlim([xl_field(1)-2.2 xl_field(2)+2.6]); ylim([yl_field(1)-1.7 yl_field(2)+1.8]); zlim([zl_field(1)-2.2 zl_field(2)+2.2]); %for recon. modes.
 ...
 if 1
    ...
    [sx_3D,sy_3D,sz_3D] = meshgrid(xg(1:5:end),yg(11),zg(1:5:end));
    Generate_FlowStreamlines(Xg,Yg,Zg,Ug0,Vg0,Wg0,sx_3D,sy_3D,sz_3D, vel_nd);
    % cross-sectional flow at z=0.
    [sx_3D,sy_3D,sz_3D] = meshgrid(xg(1:5:end),yg(1:5:end),zg(11));
    Generate_FlowStreamlines(Xg,Yg,Zg,Ug0,Vg0,Wg0,sx_3D,sy_3D,sz_3D, vel_nd); 
    shading interp
    colorbar; 
    %caxis([0 0.0001]); % for the raw and recon. flows: nt0=350.
    caxis([0 0.001]);  % for PCA modes. 
 if 1   
    ...
    % arrow size.    
    Ug_cone = Ug0./vel_raw;  Vg_cone = Vg0./vel_raw;  Wg_cone = Wg0./vel_raw;
    scal = 3; 
...
if 0
    ...
    xticks(1:4); yticks(0.4:0.2:1) %xticks(5:5:15);
...
np_rev = 600; np_bc = 100; n_rev = 1; n_bc_per_rev = 6;
%np_rev = 600; np_bc = 100; n_rev = 16; n_bc_per_rev = 6;
%np_rev = 120; np_bc = 20; n_rev = 1; n_bc_per_rev = 6;
...
if 0 % mode1 versus mode2.
    ...
    xlim([-0.025 0.025]);  ylim([-0.026 0.026]);  
    xticks(-0.02:0.02:0.02);  yticks(-0.02:0.02:0.02); 
    ...
    for i_rev = 1:n_rev % coefficient-plot envelope, over full revolution period.
        ...
        p.EdgeAlpha=1; %p.EdgeAlpha=0.05;       
    end 
    for i_bc = 1:n_bc_per_rev  % over one revolution period.
        Time_coef_plot_temp=Time_coef(1+(i_bc-1)*np_bc:i_bc*np_bc,:);
        scatter(Time_coef_plot_temp(1,mode1),Time_coef_plot_temp(1,mode2),200*i_bc,[0.9 0 0],'filled') %scatter(Time_coef_plot_temp(40,mode1),Time_coef_plot_temp(40,mode2),200*i_bc,[0.9 0 0],'filled')
    end
    if 0
    ...
    figure(4)
    clf; set(gcf,'color','w');
    hold on;  box on; daspect([1 0.0002 1])  %daspect([1 0.0007 1])
    xlim([1 1*np_rev+1]);  ylim([-0.025 0.025]);  
    xticks(1:100:1*np_rev+1); xticklabels({'0','1','2','3','4','5','6'}); yticks(-0.02:0.02:0.02); 
...
c = colorbar('Ticks',[0,clim],'FontSize',40,'TickLabels',{'0','$10^{-3}$'},'fontname','Times');  %'FontSize',65/ 80
