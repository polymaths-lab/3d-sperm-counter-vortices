function rxm_ReconFlow_AverageFar_CrossSectionYZ_Plot
...
if 1
    ...
    %//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    XL=[xl_field(1) xl_field(2)]; YL=[yl_field(1) yl_field(2)]; ZL=[zl_field(1) zl_field(2)];  % for full flow.
    %XL=[0.7 1.5]; YL=[-0.2 0.5]; ZL=[-0.2 0.5];   % for XYZ axes. 
    xlim([XL(1) XL(2)]); ylim([YL(1) YL(2)]); zlim([ZL(1) ZL(2)]); 
    view([155 15]);
    %view([155 15]);   view([-140 25]);  %views for YZ cross-sectional flows.
...
if 1   
    ...
    % Plot 2D slice planes (YZ cross-sections).
    if 1
    XL1_mat = [-5 -1.5 0 1 4];  
    ...
    if 1 % full field flow.
        ...
        sz = zeros(size(sx))+0;
        Generate_FlowStreamlines(Xg,Yg,Zg,Ug_mean,Vg_mean,Wg_mean,sx,sy,sz, vel_dim);         
    else % cross-sectional flow. <-- XL1_mat = [-5 -1.5 0 1 4].
        [sx_3D,sy_3D,sz_3D] = meshgrid(xg([31]),yg([1:1:41]),zg([1:1:41]));  %xg=[13 20 23 25 31].
    ...
    %////////////////////////////////////////////////////////////////////////////////////////////////////////////
    % Plot red cones to indicate the flow streamlines' directions.
    if 0   % for full flow (reconstructed sperm, sp#8, no boundary).
        % arrows located at the top and bottom.
        [sx_cone1,sy_cone1,sz_cone1] = meshgrid(xg([28]),yg([3 26 40]),zg([4 18 38]));  
        ...
        [sx_cone2,sy_cone2,sz_cone2] = meshgrid(xg([10 28 40]),yg([2 26 40]),zg([20]));  
       ...
       % arrows located at the right.
        [sx_cone3,sy_cone3,sz_cone3] = meshgrid(xg([10 23 40]),yg([4 26 40]),zg([22]));  
        ...
        [sx_cone4,sy_cone4,sz_cone4] = meshgrid(xg([6 23 36]),yg([22]),zg([8 15 40]));  
        ...
        [sx_cone5,sy_cone5,sz_cone5] = meshgrid(xg([6 23 36]),yg([22]),zg([8 24 40]));  
        sx_cone5(:,:,[1 3])=NaN;  sy_cone5(:,:,[1 3])=NaN;   sz_cone5(:,:,[1 3])=NaN;
        sx_cone5(:,[2 3],2)=NaN;  sy_cone5(:,[2 3],2)=NaN;   sz_cone5(:,[2 3],2)=NaN;
        
        scal1 = 20000; scal2 = 20000; scal3 = 10000; scal4 = 6000; scal5 = 5000;   
        ...
        Plot_StreamCone(fig,Xg,Yg,Zg,Ug_mean,Vg_mean,Wg_mean,sx_cone5,sy_cone5,sz_cone5,scal5)
    end
...
c.Position = [0.7 0.1 0.005 0.45];  % orientation: vertical
