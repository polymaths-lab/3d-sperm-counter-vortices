function rxm_ReconFlow_AverageFar_CrossSectionXY_Plot
...
if 1
    ...
    view([155 15]);  
    %view([155 15]); view([0 90]);  %views for XY cross-sectional flows.
...
if 1    
    ...
    % Plot 2D slice planes (XY cross-sections).
    if 1
    ZL1_mat = [-8 -3  0 3 8]; 
    ...
    if 1 % full field flow.
        ...
        sz = zeros(size(sx))+0;
        Generate_FlowStreamlines(Xg,Yg,Zg,Ug_mean,Vg_mean,Wg_mean,sx,sy,sz, vel_dim);         
    else % cross-sectional flow. <-- ZL1_mat = [-8 -3  0 3 8]    
        [sx_3D,sy_3D,sz_3D] = meshgrid(xg([1:41]),yg([1:41]),zg(37));  %zg=[5 15 21 27 37].
