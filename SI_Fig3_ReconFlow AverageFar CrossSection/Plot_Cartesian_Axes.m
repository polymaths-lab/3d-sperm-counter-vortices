function Plot_Cartesian_Axes(fig,O,C)
    ...
    % Vector: line segment.
    eff_x=0.6; eff_y=0.6; eff_z=0.6; %for far-field full flow.
    %eff_x=0.1; eff_y=0.1; eff_z=0.1; %for full flow.
    %eff_x=0.3; eff_y=0.3; eff_z=0.3; %for cross-sectional flow.
    %eff_x=0.1; eff_y=1.5; eff_z=1.5; %for 2D streamslice.
    quiver3(O(1),O(2),O(3),eff_x*C(1,1),eff_x*C(2,1),eff_x*C(3,1),'k','linewidth',4); %,'linewidth',10 % arrow indicating X axis.
    quiver3(O(1),O(2),O(3),eff_y*C(1,2),eff_y*C(2,2),eff_y*C(3,2),'k','linewidth',4); %,'linewidth',10 % arrow indicating Y axis.
    quiver3(O(1),O(2),O(3),eff_z*C(1,3),eff_z*C(2,3),eff_z*C(3,3),'k','linewidth',4); %,'linewidth',10 % arrow indicating Z axis.
    
    % Vector: arrowhead.
    eff_x=eff_x*0.8; eff_y=eff_y*0.8; eff_z=eff_z*0.8; 
    sz = 0.1; % for far-field full flow.
    %sz = 0.025; % for full flow.
    %sz = 0.05; % for cross-sectional flow.
    %sz = 0.12; % for 2D streamslice.
    [cone_X_x, cone_X_y, cone_X_z]=meshgrid(-30:30:30, -30:30:30, -30:30:30); %meshgrid(-3:3:3, -3:3:3, -1:1:1);  %this may need modification for different scales of figures.
