function rxm_ReconFlow_AverageFar_CrossSectionXZ_Plot
...
if 1
    ...
    view([155 15]);
    %view([155 15]); view([-45 10]);  %views for XZ cross-sectional flows.
...
if 1   
    ...
    % Plot 2D slice planes (XZ cross-sections).
    if 1
    YL1_mat = [-8 -3 0 3 8];
    ...
    if 1 % full field flow.
        ...
        sz = zeros(size(sx))+0;
        Generate_FlowStreamlines(Xg,Yg,Zg,Ug_mean,Vg_mean,Wg_mean,sx,sy,sz, vel_dim);         
    else % cross-sectional flow. <-- YL1_mat = [-8 -3 0 3 8].
        [sx_3D,sy_3D,sz_3D] = meshgrid(xg([1:1:41]),yg(27),zg([1:1:41]));  %yg=[5 15 21 27 37].
