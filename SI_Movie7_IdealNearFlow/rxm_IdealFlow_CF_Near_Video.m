function rxm_IdealFlow_CF_Near_Video
...
fig = 1;
figure(fig) 
v=VideoWriter('NearFlow_alpha0.6_test1','MPEG-4');
v.FrameRate=20;  %50;
open(v);     
for i_nt = 1:nt_rev+1
    i_nt
...
%================================================================================
    ...
    view([-45 20]); % for idealized model's near average flow. 
    ...
    % Plot the colorbar.
    c=colorbar; 
    c.Position=[0.66 0.35 0.005 0.57];  
    caxis([0 0.01]);      
    %c.Label.String = 'Flow velocity'; 
    text(2,-1.5,-0.6,{'Flow velocity'},'FontName','times','FontSize',fs,'rotation',90);  %'FontWeight','bold'
    c.Ticks=[0 0.01]; c.TickLabels={'0','0.01'}; 
    ...
    if 1 
        ...
        text(-0.65,0,-1.15,{'II'},'FontName','times','interpreter','latex','FontSize',fs);  %xz-plane
...
%================================================================================
    ...
    view([-45 20]); axis equal; box on; axis off;
    xlabel('x'); ylabel('y'); zlabel('z');
    % Plot Cartesian axes.
...
%================================================================================
    ...
    hold on; set(gcf,'color','w'); axis equal; view([-180 0]);  
    xlim([xl_field(1) xl_field(2)]); ylim([yl_field(1) yl_field(2)]); zlim([zl_field(1) zl_field(2)]);  
    xticks([]); yticks([]); zticks([]);
    ...
    hold on; set(gcf,'color','w'); axis equal; view([-180 90]); 
    xticks([]); yticks([]); zticks([]);
    xlim([xl_field(1) xl_field(2)]); ylim([yl_field(1) yl_field(2)]); zlim([zl_field(1) zl_field(2)]);  
    xlh=xlabel('$x_c$','interpreter','latex','FontSize',fs);
    ylh=ylabel('$y_c$','interpreter','latex','FontSize',fs);
...
%================================================================================
    ...
    ax322 = axes('Position',[0.42  0.06  0.1 0.1]);     
    hold on; set(gcf,'color','w');axis equal; box on; box off; axis off;
    view([-45 20]);
    xlabel('x'); ylabel('y'); zlabel('z');
    ...
    % Text: sub-title.
    text(-1.5,0,3.6,{'Comoving frame beat'},'FontName','times','FontSize',fs,'FontWeight','bold');   
    camlight right
    lighting gouraud  
